...
This is a helm chart for call-off-service-ui

## Configuration

### Value files

The configurations and settings that are applied to an instance of the call-off-service-ui running on k8s are determined by combining the following values files

* The `values.yaml` file in the root directory. This contains the default configurations for all instances of call-off-service-ui
...
| `resources.limits.cpu` | Max cpu required by call-off-service-ui  |
| `resources.limits.memory` | Max memory required by call-off-service-ui   |
...
| `environmentVariables` | Map of the environment variables required by call-off-service-ui. See the **Environment Variables** section above  |
| `environmentSecrets` | Map of the secrets provided as environment variables to call-off-service-ui. See the **Environment Secrets** section above   |
