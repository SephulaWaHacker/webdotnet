...
This is a helm chart for parts-service-c

## Configuration

### Value files

The configurations and settings that are applied to an instance of the parts-service-c running on k8s are determined by combining the following values files

* The `values.yaml` file in the root directory. This contains the default configurations for all instances of parts-service-c
...
| `resources.limits.cpu` | Max cpu required by parts-service-c  |
| `resources.limits.memory` | Max memory required by parts-service-c   |
...
| `environmentVariables` | Map of the environment variables required by parts-service-c. See the **Environment Variables** section above  |
| `environmentSecrets` | Map of the secrets provided as environment variables to parts-service-c. See the **Environment Secrets** section above   |
