...
This is a helm chart for __CHART__

## Configuration

### Value files

The configurations and settings that are applied to an instance of the __CHART__ running on k8s are determined by combining the following values files

* The `values.yaml` file in the root directory. This contains the default configurations for all instances of __CHART__
...
| `resources.limits.cpu` | Max cpu required by __CHART__  |
| `resources.limits.memory` | Max memory required by __CHART__   |
...
| `environmentVariables` | Map of the environment variables required by __CHART__. See the **Environment Variables** section above  |
| `environmentSecrets` | Map of the secrets provided as environment variables to __CHART__. See the **Environment Secrets** section above   |
