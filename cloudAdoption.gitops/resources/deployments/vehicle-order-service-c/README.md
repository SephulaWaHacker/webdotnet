# vehicle-order-service-c

This is a helm chart for vehicle-order-service-c

## Configuration

### Value files

The configurations and settings that are applied to an instance of the vehicle-order-service-c running on k8s are determined by combining the following values files

* The `values.yaml` file in the root directory. This contains the default configurations for all instances of vehicle-order-service-c
...
| `resources.limits.cpu` | Max cpu required by vehicle-order-service-c  |
| `resources.limits.memory` | Max memory required by vehicle-order-service-c   |
...
| `environmentVariables` | Map of the environment variables required by vehicle-order-service-c. See the **Environment Variables** section above  |
| `environmentSecrets` | Map of the secrets provided as environment variables to vehicle-order-service-c. See the **Environment Secrets** section above   |
