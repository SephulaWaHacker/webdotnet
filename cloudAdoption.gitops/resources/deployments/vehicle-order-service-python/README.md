# vehicle-order-service-python

This is a helm chart for vehicle-order-service-python

## Configuration

### Value files

The configurations and settings that are applied to an instance of the vehicle-order-service-python running on k8s are determined by combining the following values files

* The `values.yaml` file in the root directory. This contains the default configurations for all instances of vehicle-order-service-python
...
| `resources.limits.cpu` | Max cpu required by vehicle-order-service-python  |
| `resources.limits.memory` | Max memory required by vehicle-order-service-python   |
...
| `environmentVariables` | Map of the environment variables required by vehicle-order-service-python. See the **Environment Variables** section above  |
| `environmentSecrets` | Map of the secrets provided as environment variables to vehicle-order-service-python. See the **Environment Secrets** section above   |
